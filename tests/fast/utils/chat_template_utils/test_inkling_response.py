...
class FakeInklingTokenizer:
    ...
    def decode(self, token_ids, skip_special_tokens=False):
        ...
        return "".join(
            inverse[token_id] if token_id in inverse else chr(token_id - 1000)
...
def test_parses_ordered_blocks_and_keeps_recipient_separate_from_tool_name():
    ...
    assert parsed.stored_message["_miles_raw_completion_token_ids"] == completion
...
def test_malformed_and_length_cut_completions_request_retry():
    ...
    malformed_result = InklingResponseParser(tokenizer).parse(
        malformed, finish_reason="stop"
    )
    truncated_result = InklingResponseParser(tokenizer).parse(
